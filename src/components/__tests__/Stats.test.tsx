...
describe('Stats', () => {
  ...
  it('should display total bills count', () => {
    const { container } = render(<Stats bills={mockBills} filteredBills={mockBills} darkMode={false} />)
  ...
  it('should display filtered results count', () => {
    ...
    const { container } = render(<Stats bills={mockBills} filteredBills={filtered} darkMode={false} />)
    const cards = container.querySelectorAll('[style*="border-radius: 16px"]')
  ...
  it('should apply dark mode styling', () => {
    const { container } = render(<Stats bills={mockBills} filteredBills={mockBills} darkMode={true} />)
    expect(container.firstChild).toBeInTheDocument()
  })

  it('should handle empty bills array', () => {
    const { container } = render(<Stats bills={[]} filteredBills={[]} darkMode={false} />)
    const cards = container.querySelectorAll('[style*="border-radius: 16px"]')
    const firstCard = within(cards[0] as HTMLElement)
    expect(firstCard.getByText('Total Bills')).toBeInTheDocument()
